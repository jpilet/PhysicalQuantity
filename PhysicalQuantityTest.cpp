...
#include <cmath>
#include "../Algorithms/PhysicalQuantity.h"
...
TEST(PhysQuantTest, CircumferenceTest) {
  ...
  double minutes = 60.0*Angle<double>::radians(6.28318530718).degrees();
...
TEST(PhysQuantTest, WalkTest) {
  ...
  EXPECT_NEAR(velB.metersPerSecond(), velA.metersPerSecond(), 1e-5);
}

TEST(PhysQuantTest, AngleTest) {
  Angle<double> a = Angle<double>::degrees(30);
  Angle<double> b = Angle<double>::degrees(60);
  Angle<double> a2 = Angle<double>::radians(3.14159265359 /6.0);
  Angle<double> b2 = Angle<double>::radians(3.14159265359 /3.0);
...
TEST(PhysQuantTest, AngularVelocity) {
  ...
  EXPECT_NEAR(4 * 180 / 3.14159265359, base.degreesPerSecond(), 1e-5);
}
